...
@Component({
  ...
  template: `
    <div style="min-height:100vh;background:#F7F7F7;font-family:'Poppins',sans-serif;">
        ...
        <div style="max-width:1280px;margin:0 auto;padding:0 20px;position:relative;z-index:1;display:flex;flex-wrap:wrap;align-items:center;justify-content:space-between;">
          ...
          <div style="flex:1;min-width:300px;max-width:600px;padding-right:20px;">
            ...
            <div style="display:flex;flex-wrap:wrap;gap:12px;">
              <a routerLink="/signup" class="btn-hero-primary">{{ 'auth.getStarted' | translate }} — It's Free</a>
              <a routerLink="/login" class="btn-hero-secondary">{{ 'auth.signIn' | translate }}</a>
            </div>

            <p style="font-size:14px;color:rgba(0,0,0,0.6);margin-top:16px;">{{ 'auth.noAccount' | translate }}</p>
          </div>
          ...
          <div style="display:flex;flex-direction:column;gap:12px;width:100%;max-width:320px;margin-top:24px;">
            ...
            <div style="background:black;border-radius:16px;padding:16px 20px;box-shadow:0 20px 60px rgba(0,0,0,0.2);">
              <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;">
                <span style="font-size:13px;color:#FFCB05;opacity:0.8;font-weight:500;">{{ 'dashboard.totalSaved' | translate }}</span>
      ...
      <section style="padding:64px 20px;max-width:1280px;margin:0 auto;">
        <div style="text-align:center;margin-bottom:48px;">
          <h2 style="font-size:30px;font-weight:700;color:#1A1A1A;margin-bottom:12px;">{{ 'landing.features.title' | translate }}</h2>
          <p style="color:#666;max-width:500px;margin:0 auto;">{{ 'landing.features.subtitle' | translate }}</p>
        </div>
      ...
      <section style="background:black;margin:0 16px 64px;border-radius:24px;max-width:1280px;margin-left:auto;margin-right:auto;overflow:hidden;position:relative;padding:64px 24px;">
        ...
        <div style="position:relative;text-align:center;">
          ...
          <a routerLink="/signup" class="btn-cta">{{ 'auth.getStarted' | translate }}</a>
        </div>
      </section>
    ...
    <style>
      ...
      .btn-hero-primary { background: black; color: #FFCB05; font-weight: 600; padding: 16px 32px; border-radius: 12px; text-decoration: none; display: inline-block; transition: all 0.3s; box-shadow: 0 8px 30px rgba(0,0,0,0.2); font-family: 'Poppins', sans-serif; }
      .btn-hero-primary:hover { transform: scale(1.02); }
      .btn-hero-secondary { background: rgba(255,255,255,0.4); color: black; font-weight: 600; padding: 16px 32px; border-radius: 12px; text-decoration: none; border: 1px solid rgba(0,0,0,0.1); display: inline-block; transition: all 0.3s; backdrop-filter: blur(4px); font-family: 'Poppins', sans-serif; }
      .btn-hero-secondary:hover { background: rgba(255,255,255,0.6); }
      .feature-card { background: white; border-radius: 16px; padding: 24px; border: 1px solid #E5E5E5; transition: all 0.3s; }
      .feature-card:hover { border-color: #FFCB05; box-shadow: 0 8px 30px rgba(0,0,0,0.08); }
      .feature-icon { width: 48px; height: 48px; background: #FFCB05; border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 24px; margin-bottom: 16px; transition: transform 0.3s; }
      .feature-card:hover .feature-icon { transform: scale(1.1); }
      .btn-cta { background: #FFCB05; color: black; font-weight: 700; padding: 16px 40px; border-radius: 12px; text-decoration: none; display: inline-block; transition: all 0.3s; box-shadow: 0 8px 30px rgba(255,203,5,0.2); font-family: 'Poppins', sans-serif; }
